...
export function SettingsForm({ restaurant, onSaved }: { restaurant: Restaurant; onSaved: (r: Restaurant) => void }) {
  ...
  async function upload(key: 'logo_url' | 'cover_url', file: File | undefined) {
    ...
    try {
      ...
      toast.error(t('s_uploadFailed', { msg: (e as Error).message }));
    }
    setBusy(false);
  }
    ...
    <form onSubmit={save} className="mx-auto max-w-2xl space-y-5">
      ...
      <Section title={t('s_menuStyle')}>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {THEMES.map((th) => {
            ...
            return (
              ...
              >
                <div className="relative space-y-1.5 p-3" style={{ background: th.vars['--background'], color: th.vars['--foreground'] }}>
                  {selected && (
                    <span className="absolute top-2 right-2 grid size-5 place-items-center rounded-full bg-primary text-primary-foreground">
                      <Check className="size-3" />
                    </span>
                  )}
                  {th.fontUrl && <link rel="stylesheet" href={th.fontUrl} />}
                  <p className="text-xl leading-none" style={{ fontFamily: th.headingFont }}>
                    Aa
                  </p>
                  <div className="flex items-center gap-2 p-1.5" style={{ background: th.vars['--card'], borderRadius: th.radius, border: `1px solid ${th.vars['--border']}` }}>
                    <span className="size-6 shrink-0" style={{ background: th.vars['--secondary'], borderRadius: th.radius }} />
                    <span className="h-1.5 flex-1 rounded-full" style={{ background: th.vars['--muted-foreground'], opacity: 0.4 }} />
                    <span className="size-4 shrink-0 rounded-full" style={{ background: th.accent }} />
                  </div>
                </div>
                <div className="bg-card px-3 py-2">
                  <p className="text-sm font-bold">{th.name}</p>
                  <p className="line-clamp-2 text-xs text-muted-foreground">{th.description}</p>
                </div>
              </button>
            );
          })}
        ...
        <a className="inline-flex items-center gap-1.5 text-sm font-semibold text-primary" href={`#/r/${restaurant.slug}?theme=${r.theme}`} target="_blank" rel="noreferrer">
          <ExternalLink className="size-4" /> {t('s_previewStyle')}
        </a>
      </Section>

      <Section title={t('s_look')}>
        <div className="grid gap-4 sm:grid-cols-[auto_1fr]">
          <Field label={t('s_brandColour')}>
      ...
      <Section title={t('s_pricesOrdering')}>
        <div className="grid items-end gap-4 sm:grid-cols-2">
          <Field label={t('s_rielPerUsd')}>
        ...
        <label className="flex cursor-pointer items-center justify-between gap-4 rounded-xl bg-secondary/60 p-3 text-sm">
          <span>
            <b>{t('s_allowOrdering')}</b>
            <span className="block text-muted-foreground">{t('s_allowOrderingHint')}</span>
          </span>
      ...
      <Button type="submit" size="lg" className="w-full font-bold sm:w-auto" disabled={busy}>
        {busy ? t('s_saving') : t('s_saveChanges')}
      </Button>
    </form>
  );
}
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <label className="inline-flex h-8 cursor-pointer items-center gap-1.5 rounded-lg border bg-background px-2.5 text-sm font-medium hover:bg-muted">
        <ImagePlus className="size-4" /> {t('s_upload')}
