  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-1.5">
        <span className="mr-1 text-sm font-medium">Options</span>
        {PRESETS.map((p) => (
      ...
      {value.length === 0 && <p className="text-xs text-muted-foreground">No options. Add sugar level, size or add-ons if diners should choose.</p>}

      {value.map((g, gi) => (
        <div key={g.id} className="space-y-2.5 rounded-xl border p-3">
          <div className="flex items-center gap-2">
            <Input value={g.name} placeholder="Group name, e.g. Sugar level" onChange={(e) => updateGroup(gi, { name: e.target.value })} className="h-9 font-semibold" />
            <Button type="button" variant="ghost" size="icon-sm" className="text-destructive" title="Remove group" onClick={() => onChange(value.filter((_, i) => i !== gi))}>
              <Trash2 />
            </Button>
          </div>
          {languages.length > 0 && (
          ...
          <div className="flex flex-wrap gap-4 text-xs font-medium">
            <label className="flex items-center gap-2">
              <Switch size="sm" checked={g.required} onCheckedChange={(v) => updateGroup(gi, { required: v })} /> Diner must choose
            </label>
            <label className="flex items-center gap-2">
              <Switch size="sm" checked={g.multi} onCheckedChange={(v) => updateGroup(gi, { multi: v })} /> Can pick several
            </label>
          </div>
          <div className="space-y-1.5">
            {g.choices.map((c, ci) => (
              <div key={c.id} className="flex flex-wrap items-center gap-1.5 sm:flex-nowrap">
                <Input value={c.name} placeholder="Choice, e.g. Less sugar" onChange={(e) => updateChoice(gi, ci, { name: e.target.value })} className="h-8 min-w-32 flex-1 text-sm" />
                {languages.map((l) => (
                  <Input
                    key={l}
                    value={c.i18n?.[l] ?? ''}
                ...
                <Button type="button" variant="ghost" size="icon-xs" title="Remove choice" onClick={() => updateGroup(gi, { choices: g.choices.filter((_, i) => i !== ci) })}>
                  <Trash2 />
                </Button>
              </div>
            ))}
